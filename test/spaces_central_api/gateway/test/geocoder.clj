    ...
    (try
      (testing "Geocoding an address"
               (let [address {:loc-street "Sukhumvit Road" :loc-street-num "413" :loc-zip-code "10110" :loc-city "Bangkok"}  
                 ...
                 (is (= (:formatted-address result) "413 Thanon Sukhum Vit, Khwaeng Khlong Toei, Khet Khlong Toei, Krung Thep Maha Nakhon 10110, Thailand"))  
                 (is (= lng 100.5646072))
                 (is (= lat 13.7341553))))  
      (finally
    ...
    (try
      (testing "Geocoding a location"
               (let [location {:long 100.5646072 :lat 13.7341553}  
                 ...
                 (is (= lng 100.5646072))
                 (is (= lat 13.7341553))
                 (is (= (:formatted-address result) "14-493 Thanon Sukhum Vit, Khwaeng Khlong Toei, Khet Khlong Toei, Krung Thep Maha Nakhon 10110, Thailand"))))
      (finally
        (component/stop system)))))

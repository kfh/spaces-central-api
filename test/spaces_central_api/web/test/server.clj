    ...
    (try 
      (testing "Creating and retreiving an ad"
        (let [location {:location/name "Sukhumvit Road"
              ...
              res (post {:form-params (str new-ad) :content-type :transit+json :as :transit+json})
              stored-ad (:body res)]
          ...
          (let [new-res (:ad/real-estate new-ad)
            ...
            (is (= (set (:real-estate/features new-res)) (:real-estate/features stored-res)))   
    ...
    (try 
      (testing "Creating and retreiving all ads"
        (let [loc-1 {:location/name "Sukhumvit Road"
              ...
              url (str "http://" (:host web-server) ":" (:port web-server))
              post (partial http/post (str url "/api/ads"))] 
          (post {:form-params (str new-ad-1) :content-type :transit+json :as :transit+json})
          (post {:form-params (str new-ad-2) :content-type :transit+json :as :transit+json}) 
    ...
    (try 
      (testing "Creating and updating an ad"
        (let [location {:location/name "Silom Road"
              ...
              url (str "http://" (:host web-server) ":" (:port web-server))
              post (partial http/post (str url "/api/ads"))
              create-res (post {:form-params (str new-ad) :content-type :transit+json :as :transit+json})
              returned-ad (:body create-res)
              updated-ad (assoc returned-ad :ad/start-time "17:00" :ad/end-time "18:00")
              update (partial http/put (str url "/api/ads/" (:ad/public-id updated-ad))) 
              update-res (update {:form-params (str updated-ad) :content-type :transit+json :as :transit+json})
